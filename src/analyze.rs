...
/// A [`RepackSink`] that replays a source into a real filesystem **writer**,
/// but with every file's data written as zeros (so the dry run costs no data
/// RAM against a [`SizingDevice`](crate::block::sizing::SizingDevice)). The
/// writer assigns inodes / CNIDs, encodes names, and allocates blocks exactly
/// as for a real build — only the bytes differ.
struct DrySink<'a> {
    fs: &'a mut dyn crate::fs::Filesystem,
    dev: &'a mut dyn BlockDevice,
}

impl DrySink<'_> {
    fn xattrs(&mut self, path: &str, xattrs: &[XattrPair]) -> Result<()> {
        if xattrs.is_empty() {
            return Ok(());
        }
        match self
            .fs
            .set_xattrs(self.dev, std::path::Path::new(path), xattrs)
        {
            Ok(()) | Err(crate::Error::Unsupported(_)) => Ok(()),
            Err(e) => Err(e),
        }
    }
}

impl RepackSink for DrySink<'_> {
    fn put_dir(&mut self, path: &str, meta: RepackMeta, xattrs: &[XattrPair]) -> Result<()> {
        self.fs
            .create_dir(self.dev, std::path::Path::new(path), meta.to_file_meta())?;
        self.xattrs(path, xattrs)
    }
    fn put_file(
        &mut self,
        path: &str,
        ...
        let mut zero = std::io::Read::take(std::io::repeat(0u8), len);
        self.fs.create_file_streaming(
            self.dev,
    ...
    fn put_hardlink(
        ...
        // Mirror FsSink: try a real link; if the destination has no hardlink
        // concept, return false so the walker re-emits it as a file copy.
        match self.fs.hardlink(
            self.dev,
            std::path::Path::new(target),
            std::path::Path::new(path),
        ) {
...
/// One dry-run build of `source` into a fresh `F` at `size` bytes, returning
/// whether the writer's own allocator accepted it. Any error (out of space,
/// or an unrepresentable entry) counts as "doesn't fit" for the search; a
/// truly broken source surfaces when even the upper bound fails.
fn dry_build_fits<F>(source: &Source, opts: &F::FormatOpts, size: u64) -> bool
where
    F: crate::fs::FilesystemFactory,
{
    // Probing deliberately tries sizes below a filesystem's minimum, where a
    // writer may not just `Err` but *panic* (e.g. an index-out-of-bounds in a
    // format path that never expected so few blocks). Treat any panic, like
    // any error, as "doesn't fit" so the search simply moves on.
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let mut dev = crate::block::sizing::SizingDevice::new(size);
        let mut fs = match F::format(&mut dev, opts) {
            Ok(f) => f,
            Err(_) => return false,
        };
        {
            ...
            if walk_source_into_sink(source, &mut sink).is_err() {
                return false;
            }
        }
        crate::fs::Filesystem::flush(&mut fs, &mut dev).is_ok()
    }))
    .unwrap_or(false)
}

/// Ask the **writer itself** for the minimal image size that holds `source`:
/// binary-search the smallest size at which a dry-run format + populate +
/// flush of `F` succeeds. The writer assigns inodes, encodes names, builds its
/// B-trees, and allocates blocks exactly as for a real build (file data is
/// written as zeros against a sparse
/// [`SizingDevice`](crate::block::sizing::SizingDevice), so the probe is
/// metadata-only); its own out-of-space error is the authoritative "doesn't
/// fit". Rounded up to a 4 KiB block.
pub fn writer_required_size<F>(source: &Source, opts: &F::FormatOpts) -> Result<u64>
where
    F: crate::fs::FilesystemFactory,
{
    const GRAN: u64 = 4096;
    const CAP: u64 = 1 << 48; // 256 TiB sanity ceiling

    // Silence the panic output from probes that hit a writer's too-small panic
    // path (those panics are caught in `dry_build_fits`). The guard restores
    // the previous hook however this function returns.
    ...
    // The image must hold at least the file data, so start the search there
    // (a fast metadata-only walk) instead of doubling up from 1 MiB. `lo` is a
    // size that cannot fit (no room for metadata); `hi` we grow until it does.
    let data = analyze_source(source, 4096)?.total_file_bytes;
    let mut lo = data;
    let mut hi = (data + data / 8).max(1 << 20);
    while !dry_build_fits::<F>(source, opts, hi) {
        lo = hi;
        hi = hi.saturating_mul(2);
        if hi > CAP {
            return Err(crate::Error::Unsupported(
                "content too large to size automatically; pass --size".into(),
            ));
        }
    }
    // Binary-search the minimal fitting size in (lo, hi].
    while hi - lo > GRAN {
        let mid = lo + (hi - lo) / 2;
        if dry_build_fits::<F>(source, opts, mid) {
            hi = mid;
        } else {
            lo = mid;
        }
    }
    Ok(hi.div_ceil(GRAN) * GRAN)
}
...
mod tests {
    ...
    #[test]
    fn writer_required_size_fits_a_real_build() {
        // The writer-determined size must be one the *real* build accepts, and
        // one block smaller must not — proving it's the true minimum.
        ...
        for i in 0..40 {
            std::fs::write(dir.path().join(format!("sub/f{i}.txt")), vec![1u8; 5000]).unwrap();
        }
        let src = Source::HostDir(dir.path().to_path_buf());
        let opts = FormatOpts::default();
        let size = writer_required_size::<HfsPlus>(&src, &opts).unwrap();

        // A real format + populate at `size` succeeds (the dry-run size is
        // sufficient for the actual build).
        let mut dev = MemoryBackend::new(size);
        let mut fs = HfsPlus::format(&mut dev, &opts).unwrap();
        crate::repack::populate_fs_from_source(&mut dev, &mut fs, &src).expect("must fit");
        crate::fs::Filesystem::flush(&mut fs, &mut dev).unwrap();

        // Minimal to within the block-rounding: two blocks smaller does not fit.
        assert!(
            !dry_build_fits::<HfsPlus>(&src, &opts, size - 2 * 4096),
            "size {size} is not tight: {} also fit",
            size - 2 * 4096
        );
        // And far under the old `2× + 64 MiB` heuristic (tiny content here).
        assert!(
            size < 16 * 1024 * 1024,
            "expected a tight image, got {size}"
        );
    }
