...
impl<'a> Ext2FileHandle<'a> {
    ...
    /// Blocks charged to the inode for its external xattr block (0 or
    /// 1). `i_blocks` covers it — `ext4_inode_is_fast_symlink` and
    /// e2fsck both rely on that — but recomputing `i_blocks` from the
    /// data map alone dropped it, so every rw write to a file carrying
    /// xattrs under-reported `i_blocks` by one block.
    fn xattr_blocks(&self) -> u64 {
        u64::from(self.staged_inode().file_acl != 0)
    }
    ...
    fn recompute_blocks_512(&mut self) -> Result<()> {
        ...
        let total_sectors = (data + meta + self.xattr_blocks()) * (bs as u64 / 512);
    ...
    fn recompute_blocks_512_extent(&mut self) -> Result<()> {
        ...
        let sectors = (data + meta + self.xattr_blocks()) * (bs / 512);
