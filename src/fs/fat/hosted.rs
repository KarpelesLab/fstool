...
// These were sibling modules when this file was the module root; it is
// now one half of it, so name them through the parent.
// The submodules stay part of this half of the backend — they reach into
// `Fat32`'s internals, so they must remain its descendants. `super`
// re-exports them, keeping the public paths (`fs::fat::table`, …) as they
// have always been.
#[path = "boot.rs"]
pub mod boot;
#[path = "dir.rs"]
...
/// (the exact path is [`size_plan::FatSizePlan`]). Budgets ~1 KiB per cluster at the
