//! The hosted half of the littlefs backend: the `Filesystem`
//! implementation, built on a heap.
//!
//! The format itself is described in the [module
//! docs](crate::fs::littlefs); this file is what a machine with an
//! allocator does with it. A volume is opened over a
//! [`BlockDevice`](crate::block::BlockDevice) rather than a
//! [`FlashDriver`](super::FlashDriver), metadata pairs are replayed into
//! `Vec<Entry>` views, and the block allocator holds an exact in-use bitmap
//! for the whole volume — all of which is right for building and converting
//! images and wrong for a microcontroller, which is what
//! [`Volume`](super::Volume) is for.
//!
...
// These were sibling modules when this file was the module root; it is now
// one half of it, so they are mounted here by path. They reach into
// `LittleFs`'s internals, so they must stay its descendants.
#[path = "alloc.rs"]
mod alloc;
#[path = "ctz.rs"]
mod ctz;
...
// The on-disk tag format and the disk-version constants are the two things
// both halves of the backend speak, and they live in the module root. The
// re-export keeps `super::tag` resolving for the submodules above, which
// were written when this file was that root.
pub(super) use super::{
